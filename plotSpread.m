...
if isempty(opt.sizedots) 
    sizedots=14;
else
    sizedots=opt.sizedots;
end 
            ...
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = plot(ah,data(currentIdx,1),...
                        ...
                        'lineStyle','none',...
                        'DisplayName',plotLabels{iData,iCategory}, 'Markersize', sizedots, 'Markerfacecolor', 'w');
                case 'flipped'
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues,m,'+k','Color','k','MarkerSize',15);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues,md,'sg','MarkerSize',15);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues,m,'+k','Color','k','MarkerSize',15);
        ...
        case 'flipped'
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues,'+k','Color','k','MarkerSize',15);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues,'sg','MarkerSize',15);
            end
            if opt.showMM == 4
                mh = plot(ah,m,opt.xValues,'+k','Color','k','MarkerSize',15);
