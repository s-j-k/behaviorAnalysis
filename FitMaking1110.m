...
%% plot individual fitted d' in separate figure
colors=[[187, 186, 183]; [255, 195, 185]]./255;
plotyes=1; %1 if you want to get figure with individual fits, 0 if not
exampleanimalsR=0; %to plot example Reinforced animals
clear xf yf xfplat yfplat yfplathalf xfplathalf mousename
...
%         plot(xfplathalf(ii,:), yfplathalf(ii,:), 'go') %for halfplateau       
        ...
        saveas(gi,['SM' num2str(micetoeval(i)) '.fig']);
        saveas(gi,['SM' num2str(micetoeval(i)) '.tiff']);
%         figure(1012)
%         subplot(1,2,1)
%         hold all
%         h1=plot(fitresultR{ii},'-',xf(2:end),yf(2:end),'.');
...
%% plot all the fitted curves
figure; hold on;
for ii=1:length(fitresultR)
        %Reinforced
    xf=[1 xtrialsR(ii,1:maxblock)]'; %x data for fit
    yf=[0 dp_sli100R(ii,1:maxblock)]'; %y data for fit
...
%      h1=plot(fitresultR{ii},'k-',xf(2:end),yf(2:end),'k.'); % with points
    h1=plot(fitresultR{ii}); % without points
    xlim([0 4000])
    legend off
end

%% all the fitted curve coefficients for Reinforced
coeffMat=zeros(length(fitresultR),4);
 % for reinforced
for ii=1:length(fitresultR)
    temp=fitresultR{ii};
    if isempty(temp)==1
        ...
        coeffMat(ii,1)=temp.a;
        coeffMat(ii,2)=temp.b;
        coeffMat(ii,3)=temp.c;
        coeffMat(ii,4)=temp.d;
    end
end

%% check wtf the coefficient d is

y1 = a+(b-a)/2; % y coordinate of base point
x1 = 1386;% x coordinate of base point
m = 1/126.9; % slope (this is the value of d)
% define some x values between zero and 10
x = linspace(x1-100,x1+100,10);
% evalualte the point slope formulae
y = m*(x-x1) + y1;
% plot the fitted line
plot(x1,y1,'o',x,y)

% from this plot we can see d is the value of the slope at c, whihc is the
% halfway point of the sigmoid function
