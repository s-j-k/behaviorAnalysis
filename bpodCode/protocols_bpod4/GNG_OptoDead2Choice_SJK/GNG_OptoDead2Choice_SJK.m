function GNG_OptoDead2Choice_SJK
% this is for 11/6/24 when we ran the 3rd and 4th dead period inactivatino
...
% second dead happens 50ms into the dead period until the first lick
...
FullTrialOptoGraph = bar(gca, c, z3); title('Third Dead Opto'); xlabel('Outcome'); ylabel('% Correct'); ylim([0 110]); yticks(0:20:110); % performance in probe
...
ToneOptoGraph = bar(gca, c, z6); title('Fourth Dead Opto'); xlabel('Outcome'); ylabel('% Correct'); ylim([0 110]); yticks(0:20:110); % performance
...
elseif S.context(currentTrial) == 1 % Third Dead context 
    sma = AddState(sma, 'Name', 'PreTrial', ... % Pre trial period ensuring no activity for 2 seconds
        'Timer', waitfornolick, ...
    ...
    sma = AddState(sma, 'Name', 'Stimulus', ... % Tone presentation
        ...
        'OutputActions', {'SoftCode', Stimulus});  
    sma = AddState(sma, 'Name', 'SmallFirstDead', ... % 50ms dead period
        'Timer', smallestdeadperiod, ...
        'StateChangeConditions', {'Tup', 'MiddleDead'}, ...
        'OutputActions', {}); 
    sma = AddState(sma, 'Name', 'MiddleDead', ... % 100ms dead period
        'Timer', smalldeadperiod, ...
        'StateChangeConditions', {'Tup', 'LastDead'}, ...
        'OutputActions', {'BNC1', LightON}); 
    sma = AddState(sma, 'Name', 'LastDead', ... % 50ms dead period
        'Timer', smallestdeadperiod, ...
        'StateChangeConditions', {'Tup', 'WaitForLick'}, ...
        'OutputActions', {});     
    sma = AddState(sma, 'Name', 'WaitForLick', ... % Response period
...
% context 5, light on fourth dead 150ms into deadperiod light
% comes on
elseif S.context(currentTrial) == 5 % second dead OPTO context 
    ...
    sma = AddState(sma, 'Name', 'Stimulus', ... % Tone presentation
        ...
        'OutputActions', {'SoftCode', Stimulus});  
    sma = AddState(sma, 'Name', 'DeadFirst', ... % 150ms dead period 
        'Timer', threequartersdead, ...
        'StateChangeConditions', {'Tup', 'DeadSecond'}, ...
        'OutputActions', {}); 
    sma = AddState(sma, 'Name', 'DeadSecond', ... % 50ms dead period turn the light on
        'Timer', smallestdeadperiod, ...
        'StateChangeConditions', {'Tup', 'WaitForLick'}, ...
        'OutputActions', {'BNC1', LightON}); 
    sma = AddState(sma, 'Name', 'WaitForLick', ... % Response period
