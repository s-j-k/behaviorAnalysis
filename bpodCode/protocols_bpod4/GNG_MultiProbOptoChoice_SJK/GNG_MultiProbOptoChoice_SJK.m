function GNG_MultiProbOptoChoice_SJK
...
onlyoptointone = false; % this will replace 1/2 of the full trial inactivation 
onlyoptoinchoice = false; % this will replace 1/2 of the full trial inactivation. 
% if both of the following lines are set to true, then there will only be
% opto inactivation during the tone and during choice period
% if tone is set to false, then there will be opto inactivation for half of
% the opto trials for the full trial, and the other half of opto trials for
% just choice

if onlyoptointone==1 && onlyoptoinchoice~=1 && opto>=0.3
    warning('This protocol has trials where the full trial will be inactivated and trials where the tone will be inactivated.')
    warning('This is not advised unless there is a lower portion of opto trials, as the animal can lose motivation because it cannot hear the tone on several trials.')
end
...
% CONTEXT (S.context) --> 0=PROBE 1=OPTO 2=NORMAL
if onlyoptointone && onlyoptoinchoice %opto for Tone AND choice, no full trial
...
elseif onlyoptointone && onlyoptoinchoice~=1 %opto for Tone AND Full Trial
    warning('This protocol has trials where the full trial will be inactivated and trials where the tone will be inactivated.')
    warning('This is not advised unless there is a lower portion of opto trials, as the animal can lose motivation because it cannot hear the tone on several trials.')
...
elseif onlyoptoinchoice && onlyoptointone~=1   %opto for Choice AND Full Trial
...
disp([num2str(sum(ctxx==1)) ' Full Trial opto, ' num2str(sum(trialT==5)) ' target, ' num2str(sum(trialT==6)) ' foil.']);
disp([num2str(sum(ctxx==5)) ' Tone Only opto, ' num2str(sum(trialT==11)) ' target, ' num2str(sum(trialT==12)) ' foil.']);
disp([num2str(sum(ctxx==6)) ' Choice Only opto, ' num2str(sum(trialT==13)) ' target, ' num2str(sum(trialT==14)) ' foil.']);
...
FullTrialOptoGraph = bar(gca, c, z3); title('Full Trial Opto'); xlabel('Outcome'); ylabel('% Correct'); ylim([0 110]); yticks(0:20:110); % performance in probe
...
ToneOptoGraph = bar(gca, c, z6); title('Tone Opto'); xlabel('Outcome'); ylabel('% Correct'); ylim([0 110]); yticks(0:20:110); % performance
...
ChoiceOptoGraph = bar(gca, c, z7); title('Choice Opto'); xlabel('Outcome'); ylabel('% Correct'); ylim([0 110]); yticks(0:20:110); % performance
...
switch TrialTypes(currentTrial) % Determine trial-specific state matrix fields
    ...
    case 4 % Probe-No-Go trial  
        ...
        INResponse = 'Punish';
        NOResponse = 'CorrectReject'; 
    case 5 % Full Opto-GO trial
        Stimulus = 1; % GoTone
        INResponse = 'OpenValve';
        NOResponse = 'Miss';
        LightON = 1;
    case 6 % Full Opto-No-Go trial
        Stimulus = 2; %NoGoTone
    ...
    case 10 % Catch reinforced light ON (No foil)
        ...
        NOResponse = 'CorrectReject'; 
        LightON = 1;    
    case 11 % Tone Opto-GO trial
        Stimulus = 1; % GoTone
        INResponse = 'OpenValve';
        NOResponse = 'Miss';
        LightON = 1;
    case 12 % Tone Opto-No-Go trial
        Stimulus = 2; %NoGoTone
        INResponse = 'Punish';
        NOResponse = 'CorrectReject'; 
        LightON = 1;
    case 13 % Choice Opto-GO trial
        Stimulus = 1; % GoTone
        INResponse = 'OpenValve';
        NOResponse = 'Miss';
        LightON = 1;
    case 14 % Choice Opto-No-Go trial
        Stimulus = 2; %NoGoTone
...
if S.context(currentTrial) == 2 % Reinforced context 
    sma = AddState(sma, 'Name', 'PreTrial', ... % Pre trial period ensuring no activity for 1 second
        'Timer', waitfornolick, ...
...
elseif S.context(currentTrial) == 1 % OPTO context 
    ...
    sma = AddState(sma, 'Name', 'OpenValve', ... % Open valve for reward
        ...
        'OutputActions', {'ValveState', 1, 'BNC1', LightON}); 
    sma = AddState(sma, 'Name', 'DrinkingLightON', ... % 1st second of drinking with light
        ...
        'OutputActions', {'ValveState', 2, 'BNC1', LightON}); 
...
% context 5, light on only during tone
elseif S.context(currentTrial) == 5 % Tone OPTO context 
    sma = AddState(sma, 'Name', 'PreTrial', ... % Pre trial period ensuring no activity for 2 seconds
        'Timer', waitfornolick, ...
        'StateChangeConditions', {'Tup', 'LightON', 'Port2In', 'Stop'}, ... % if no action, trigger light ON; if action, stop period
        'OutputActions', {}); 
...
% context 6, light on after first lick
elseif S.context(currentTrial) == 6 % Choice OPTO context 
    sma = AddState(sma, 'Name', 'PreTrial', ... % Pre trial period ensuring no activity for 2 seconds
        'Timer', waitfornolick, ...
        'StateChangeConditions', {'Tup', 'Stimulus', 'Port2In', 'Stop'}, ... % if no action, stimulus activated; if action, stop period
        'OutputActions', {}); 
