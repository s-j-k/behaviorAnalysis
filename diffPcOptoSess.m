...
for jj=2:size(mgbTempTestsOnly,1)
    mgbTempTestsOnly{jj,39}=mgbTempTestsOnly{jj,28}-mgbTempTestsOnly{jj,27};
    mgbTempTestsOnly{jj,40}=mgbTempTestsOnly{jj,30}-mgbTempTestsOnly{jj,29};
    mgbTempTestsOnly{jj,41}=mgbTempTestsOnly{jj,32}-mgbTempTestsOnly{jj,31};
end
diffFull=NaN; diffTone=NaN; diffChoice=NaN;
for jj=2:size(mgbTempTestsOnly,1)
    diffFull=cat(1,diffFull,mgbTempTestsOnly{jj,39});
    diffTone=cat(1,diffTone,mgbTempTestsOnly{jj,40});
    diffChoice=cat(1,diffChoice,mgbTempTestsOnly{jj,41});
end
diffChoiceTrunc=diffChoice(~isnan(diffChoice));
diffChoiceTrunc=diffChoiceTrunc(1:length(diffTone(~isnan(diffTone))));
diffFullTrunc=diffFull(~isnan(diffChoice));
diffFullTrunc=diffFullTrunc(1:length(diffTone(~isnan(diffTone))));
catMGBDiff=[diffFullTrunc diffTone(~isnan(diffTone)) diffChoiceTrunc];
hold on;
