...
cTFLickMat=NaN(300,60);
    ...
    FAFig=figure;
    subplot(5,1,1);scatter(rTFLickMat,allMiceRTFoil(idx,:)',sz,licksColor,'filled');
    hold on;scatter(lickLatRTF,allMiceRTFoilIdx(idx,:)',sz,reinfcolor,'filled');title('Light off, FA');
    xlim([0 3]);ylim([1 300]);scatter(zeros(1,length(crIdx)),idxAllMice{8,3}(count,crIdx)',sz,reinfcolor);
    ylabel('Trial');
    subplot(5,1,2);scatter(tFLickMat,allMiceToneFoil(idx,:)',sz,licksColor,'filled');
    hold on;scatter(lickLatToneF,allMiceToneFoilIdx(idx,:)',sz,optocolor,'filled'); title('Stimulus opto');
    xlim([0 3]);ylim([1 300]);scatter(zeros(1,length(crIdx)),idxAllMice{8,3}(count,crIdx)',sz,reinfcolor);
    ylabel('Trial');
    subplot(5,1,3);scatter(cTFLickMat,allMiceCTFoil(idx,:)',sz,licksColor,'filled');
    hold on;scatter(lickLatCF,allMiceChoiceTFoilIdx(idx,:)',sz,optocolor,'filled');title('Choice opto');
    xlim([0 3]);ylim([1 300]);scatter(zeros(1,length(crIdx)),idxAllMice{8,3}(count,crIdx)',sz,reinfcolor);
    ...
    FAFullFig=figure;
    subplot(5,1,1);hold on;scatter(rFFLickMat,allMiceRFFoil(idx,:)',sz,licksColor,'filled');
    scatter(lickLatRFF,allMiceFullFoilIdx(idx,:)',sz,reinfcolor,'filled');title('Light off, FA');xlim([0 2]);ylim([1 300]);
