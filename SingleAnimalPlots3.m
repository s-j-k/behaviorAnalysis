function [] = SingleAnimalPlots3(datan, dataNew, animals, DataDay)
...
%     ProbeFa = dataNew(i).probefa(~isnan(dataNew(i).probefa));
    ProbeHit = DataDay(i).probehit;
    ProbeFa = DataDay(i).probefa;
    ...
    subplot(3, 3, 3)
    dprimeProbe = DataDay(i).dprimeprobe;
...
%     %%% mean Lick latency trial wise
    ...
    for m = 1:length(dataNew(i).HitLicks)
        for  j= 1:length(dataNew(i).HitLicks{m})
            if iscell(dataNew(i).HitLicks{m}{j})
                Licks = cell2mat(dataNew(i).HitLicks{m}{j});
...
%     ylabel('#trials')
    ...
    for m = 1:length(dataNew(i).FALicks)
        for  j= 1:length(dataNew(i).FALicks{m})
            if iscell(dataNew(i).FALicks{m}{j})
                Licks_FA = cell2mat(dataNew(i).FALicks{m}{j});
...
%     xlabel('time(s)')
    ...
    for m = 1:length(dataNew(i).CRLicks)
        for  j= 1:length(dataNew(i).CRLicks{m})
            if iscell((dataNew(i).CRLicks{m}{j}))
                Licks_CR = cell2mat(dataNew(i).CRLicks{m}{j});
 ...
 subplot(3, 3, 7)
    ...
    for k = 1:length(dataNew(i).HitLicks)
        for j = 1:length(dataNew(i).HitLicks{k})
            if iscell(dataNew(i).HitLicks{k}{j})
                idx = find(cell2mat(dataNew(i).HitLicks{k}{j})>0);
                HitData = cell2mat(dataNew(i).HitLicks{k}{j});
                LickLatencyHit(tr)= HitData(idx(1));
                tr = tr+1;
            else
            end
        end
    ...
    for k = 1:length(dataNew(i).FALicks)
        for j = 1:length(dataNew(i).FALicks{k})
            if iscell(dataNew(i).FALicks{k}{j})
                idx = find(cell2mat(dataNew(i).FALicks{k}{j})>0);
                FAData = cell2mat(dataNew(i).FALicks{k}{j});
                LickLatencyFA(tr)= FAData(idx(1));
                tr = tr+1;
            else
            end
        end
    ...
    sgtitle(animals(i))
%     DirectoryPath ='T:\su\Figures';
%     figName=strcat(string(animals(i)), 'summaryPlot.pdf');
%     whereToStore=fullfile(DirectoryPath,figName);
