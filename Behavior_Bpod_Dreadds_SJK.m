function Behavior_Bpod_Dreadds_SJK(cohort)
...
for nbsubj = 1:nSubj % through animals
    ...
    subjPath = [path subj '\GNGMultiAmp\Session Data\'];
...
%                 if
%                 ~isnan(SessionData.RawEvents.Trial{1,j}.States.DrinkingLightON)      
                if ~isnan(SessionData.RawEvents.Trial{1,j}.States.Drinking)% for dreadds data
...
%                 elseif ~isnan(SessionData.RawEvents.Trial{1,j}.States.PunishLightON)
                elseif ~isnan(SessionData.RawEvents.Trial{1,j}.States.Punish)% for dreadds data
...
% rates 17 - 18 - choice 
    ...
    [mgbDays, icDays, expRange,dreadds] = getExpDays(path, nbsubj);
...
%     % make a grid of two by three, where each row is a targeted area (MGB or
    ...
    if sum(mgbDays)==0
        if sum(dreadds)>0
            ...
            eee(1).FaceColor='flat'; eee(2).FaceColor='flat'; eee(1).CData=[reinfcolor;reinfcolor]; hold on;
            scatter(repmat(eee(1).XEndPoints(1),size(rates(expRange(dreadds==1),1),1),1),(rates(expRange(dreadds==1),1)),'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',reinfcolor);
            scatter(repmat(eee(1).XEndPoints(2),size(rates(expRange(dreadds==1),1),1),2),(rates(expRange(dreadds==1),2)),'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',reinfcolor);
            eee(2).CData=[optocolor;optocolor];ylim([0 1]);
