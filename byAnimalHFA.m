...
for jj=2:size(icDataTestsOnly,1)
    rhittemp=icDataTestsOnly{jj,18}; % full IC
    rfatemp=icDataTestsOnly{jj,19};
    ohittemp=icDataTestsOnly{jj,20};
    ofatemp=icDataTestsOnly{jj,21};
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp);
    rfa(jj-1)=nanmean(rfatemp);
...
scatter(repmat(ppp(1).XEndPoints(4),size(ofa,2),2), ...
    ofa,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',optocolor);
hitAndFa = [rhit(1) rfa(1) rhit(2) rfa(2)];
ohitAndFa = [ohit(1) ofa(1) ohit(2) ofa(2)];
line(xVector,hitAndFa, 'LineWidth', 0.5, 'Color', [0 0 0]);
line(xoVector,ohitAndFa,'LineWidth', 0.5, 'Color', [0 0 0]);
[h,pHit,ci,stats] = ttest2(rhit,ohit);
[h,pFA,ci,stats] = ttest2(rfa,ofa);
icDataTestsOnly{2,39}=[pHit pFA];
sigstar({[1,3],[2,4]}, [pHit pFA])
ylabel('rate');
...
rhit=NaN;ohit=NaN;rfa=NaN;ofa=NaN;
for jj=2:size(icDataTestsOnly,1)
    rhittemp=icDataTestsOnly{jj,18}; % tone IC
    rfatemp=icDataTestsOnly{jj,19};
    ohittemp = icDataTestsOnly{jj,22};
    ofatemp = icDataTestsOnly{jj,23};
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp);
    rfa(jj-1)=nanmean(rfatemp);
...
rhit=NaN;ohit=NaN;rfa=NaN;ofa=NaN;
for jj=2:size(icDataTestsOnly,1)
    rhittemp=icDataTestsOnly{jj,18}; % choice IC
    rfatemp=icDataTestsOnly{jj,19};
    ohittemp = icDataTestsOnly{jj,24};
    ofatemp = icDataTestsOnly{jj,25};
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp);
    rfa(jj-1)=nanmean(rfatemp);
...
hold on;
for jj=2:size(icDataTestsOnly,1)
    rhittemp=icDataTestsOnly{jj,18}; % full MGB
    rfatemp=icDataTestsOnly{jj,19};
    ohittemp=icDataTestsOnly{jj,20};
    ofatemp=icDataTestsOnly{jj,21};
    rhittemp(isnan(ohit))=nan;
...
rhit=NaN;ohit=NaN;rfa=NaN;ofa=NaN;
for jj=2:size(icDataTestsOnly,1)
    rhittemp=icDataTestsOnly{jj,18}; % tone MGB
    rfatemp=icDataTestsOnly{jj,19};
    ohittemp = icDataTestsOnly{jj,22};
    ofatemp = icDataTestsOnly{jj,24};
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp);
    rfa(jj-1)=nanmean(rfatemp);
...
ylim([-0.5 3.5]);
dprimeFig.Position(3:4)=[725 200];
saveas(gcf,['By_Animal_T_IC_dp_Opto_Avg']);
saveas(gcf,['By_Animal_T_IC_dp_Opto_Avg.png']);
%%
% clear dprimeFig
...
clear dprimeFIg
dprimeFig=figure(21);xVector = [1 2 1 2];
hold on;
clear rhittemp rfatemp rfatemp2 rhittemp2 ohittemp ofatemp
for jj=2:size(icDataTestsOnly,1)
    ...
    if length(icDataTestsOnly{jj,19})>4
        rfatemp(:,jj-1)=icDataTestsOnly{jj,19}(1:4);
    else 
        rfatemp(:,jj-1)=icDataTestsOnly{jj,19}; % full MGB
    end
    if length(icDataTestsOnly{jj,20})>4
        ohittemp(:,jj-1)=icDataTestsOnly{jj,20}(1:4);
    else
        ohittemp(:,jj-1)=icDataTestsOnly{jj,20}; % full MGB
    end
    if length(icDataTestsOnly{jj,21})>4
        ofatemp(:,jj-1)=icDataTestsOnly{jj,21}(1:4);
    else
        ofatemp(:,jj-1)=icDataTestsOnly{jj,21}; % full MGB
    end
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp(:,jj-1));
...
[h,p,ci,stats] = ttest2(mean(dp_r),mean(dp_oFull));
sigstar({[1,2]}, p)
% dprimeVals = [dp_r_An(1) dp_oFull_An(1) dp_r_An(2) dp_oFull_An(2)];
dprimeVals = [dp_r_An(1) dp_oFull_An(1)];
xVector=xVector(~isnan(dprimeVals));
dprimeVals=dprimeVals(~isnan(dprimeVals));
line([1 2],dprimeVals(1:2), 'LineWidth', 0.5, 'Color', [0 0 0]);

icDataTestsOnly{2,39} = [dp_r_An; dp_oFull_An;p];
title('Full');ylabel('d prime');ylim([-1 6]);
xticklabels({'','light off','light on'});
...
for jj=2:size(icDataTestsOnly,1)
    if length(icDataTestsOnly{jj,18})>4
        ...
        rhittemp(:,jj-1)=icDataTestsOnly{jj,18}; % tone IC
    end
    if length(icDataTestsOnly{jj,19})>4
        rfatemp(:,jj-1)=icDataTestsOnly{jj,19}(1:4);
    else
    ...
    if length(icDataTestsOnly{jj,22})>4
        ohittemp(:,jj-1)=icDataTestsOnly{jj,22}(1:4);
    else
        ohittemp(:,jj-1)=icDataTestsOnly{jj,22}; 
    end
    if length(icDataTestsOnly{jj,23})>4
        ofatemp(:,jj-1)=icDataTestsOnly{jj,23}(1:4);
    else
        ofatemp(:,jj-1)=icDataTestsOnly{jj,23};
    end
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp(:,jj-1));
...
dprimeVals = [dp_r_An(1) dp_oFull_An(1)];
line([1 2],dprimeVals(1:2), 'LineWidth', 0.5, 'Color', [0 0 0]);
icDataTestsOnly{2,40} = [dp_r_An; dp_oFull_An;p];
title('Stimulus');ylim([-1 6]);
xticklabels({'','light off','light on'});
...
for jj=2:size(icDataTestsOnly,1)
    if length(icDataTestsOnly{jj,18})>4
        ...
        rhittemp(:,jj-1)=icDataTestsOnly{jj,18}; % choice MGB
    end
    if length(icDataTestsOnly{jj,19})>4
        rfatemp(:,jj-1)=icDataTestsOnly{jj,19}(1:4);
    else
        rfatemp(:,jj-1)=icDataTestsOnly{jj,19}; % choice IC
    end
    if length(icDataTestsOnly{jj,24})>4
        ohittemp(:,jj-1)=icDataTestsOnly{jj,24}(1:4);
    else
        ohittemp(:,jj-1)=icDataTestsOnly{jj,24}; 
    end
    if length(icDataTestsOnly{jj,25})>4
        ofatemp(:,jj-1)=icDataTestsOnly{jj,25}(1:4);
    else
        ofatemp(:,jj-1)=icDataTestsOnly{jj,25}; 
    end
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp(:,jj-1));
...
dprimeVals = [dp_r_An(1) dp_oFull_An(1)];
line([1 2],dprimeVals(1:2), 'LineWidth', 0.5, 'Color', [0 0 0]);
icDataTestsOnly{2,41} = [dp_r_An;dp_oFull_An;p];
title('Choice');ylim([-1 6]);
xticklabels({'','light off','light on'});

dprimeFig.Position(3:4)=[725 225];
saveas(gcf,['Window Animal_T_IC_dp_bar_Opto']);
saveas(gcf,['Window Animal_T_IC_dp_bar_Opto.png']);
saveas(gcf,['Window Animal_T_IC_dp_bar_Opto.pdf']);
