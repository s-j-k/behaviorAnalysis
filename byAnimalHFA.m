function byAnimalHFA(allDataTestsOnly,allDataCtlOnly,mgbTempTestsOnly,icDataTestsOnly,reinfcolor,optocolor)
...
for jj=2:size(icDataTestsOnly,1)
    ...
    if length(icDataTestsOnly{jj,12})>4
        ohittemp(:,jj-1)=icDataTestsOnly{jj,12}(1:4);
    else
        ohittemp(:,jj-1)=icDataTestsOnly{jj,12}; % full MGB
    end
    if length(icDataTestsOnly{jj,13})>4
        ofatemp(:,jj-1)=icDataTestsOnly{jj,13}(1:4);
    else
        ofatemp(:,jj-1)=icDataTestsOnly{jj,13}; % full MGB
    end
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp(:,jj-1));
