...
scatter(repmat(ppp(1).XEndPoints(1),size(rhit,2),1), ...
    rhit,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',reinfcolor,'jitter','on', 'jitterAmount',0.2);
scatter(repmat(ppp(1).XEndPoints(2),size(rfa,2),1), ...
    rfa,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',reinfcolor,'jitter','on', 'jitterAmount',0.2);
scatter(repmat(ppp(1).XEndPoints(3),size(ohit,2),1), ...
    ohit,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',optocolor,'jitter','on', 'jitterAmount',0.2);
...
sigstar({[1,3],[2,4]}, [pHit pFA]);box off;
ylabel('rate');
...
scatter(repmat(ppp(1).XEndPoints(4),size(ofa,2),1), ...
    ofa,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',optocolor,'jitter','on', 'jitterAmount',0.2);
hitAndFa = [rhit(1) rfa(1) rhit(2) rfa(2) rhit(3) rfa(3) ...
    rhit(4) rfa(4) ohit(5) ofa(5) ohit(6) ofa(6)];
ohitAndFa = [ohit(1) ofa(1) ohit(2) ofa(2) ohit(3) ofa(3) ...
    ohit(4) ofa(4) ohit(5) ofa(5) ohit(6) ofa(6)];
line([1 2],hitAndFa(1:2), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],hitAndFa(3:4), 'LineWidth', 0.5, 'Color', [0 0 0]);
...
[h,pHit,ci,stats] = ttest2(rhit,ohit);
[h,pFA,ci,stats] = ttest2(rfa,ofa);
icDataTestsOnly{2,39}=[pHit pFA];box off;
sigstar({[1,3],[2,4]}, [pHit pFA])
ylabel('rate');
...
[h,pHit,ci,stats] = ttest2(rhit,ohit);
[h,pFA,ci,stats] = ttest2(rfa,ofa);
icDataTestsOnly{2,40}=[pHit pFA];box off;
sigstar({[1,3],[2,4]}, [pHit pFA])
ylabel('rate');
...
for jj=2:size(icDataTestsOnly,1)
    ...
    if length(icDataTestsOnly{jj,21})>4
        ...
        ofatemp(:,jj-1)=icDataTestsOnly{jj,21}; % full ic
    end
    rhittemp(isnan(ohit))=nan;
    rfatemp(isnan(ofa))=nan;
    rhit(jj-1)=nanmean(rhittemp(:,jj-1));
...
[dp_r,c_r]=dprime_simple(rhittemp2,rfatemp2);
for qq=1:size(ohittemp2,1)
    for ww=1:size(ohittemp2,2)
        if ohittemp2(qq,ww) > 0.99
            ohittemp2(qq,ww)=(139/140); % correct for hit rate. else the d' values are going to be super high
        else
        end
    end
end
...
line([1 2],dprimeVals(9:10), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(11:12), 'LineWidth', 0.5, 'Color', [0 0 0]);
icDataTestsOnly{2,39} = [dp_r_An; dp_oFull_An];
title('Full');ylabel('d prime');ylim([-1 4]);
xticklabels({'','light off','light on'});
...
% end
[dp_r,c_r]=dprime_simple(rhit,rfa);
[dp_ocFull,c_oFull]=dprime_simple(ohit, ofa);
subplot(1,3,3); 
dp_re=nansem(dp_r');
dp_rm=nanmean(dp_r');
hold on;
...
dp_oFull_An=mean(dp_ocFull);
dp_r_An=mean(dp_r);
ddd=bar([dp_rm odp_rm]); hold on;
ddd(1).FaceColor='flat';ddd(1).CData=[reinfcolor;optocolor];hold on;
scatter(repmat(ddd(1).XEndPoints(1),size(dp_r,2),1), ...
    dp_r,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',reinfcolor);
scatter(repmat(ddd(1).XEndPoints(2),size(dp_ocFull,2),1), ...
    dp_ocFull,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',optocolor);
[h,p,ci,stats] = ttest2(dp_r,dp_ocFull);
sigstar({[1,2]}, p)
p
dprimeVals = [dp_r(1) dp_ocFull(1) dp_r(2) dp_ocFull(2) dp_r(3) dp_ocFull(3) ...
    dp_r(4) dp_ocFull(4) dp_r(5) dp_ocFull(5) dp_r(6) dp_ocFull(6)];
line([1 2],dprimeVals(1:2), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(3:4), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(5:6), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(7:8), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(9:10), 'LineWidth', 0.5, 'Color', [0 0 0]);
line([1 2],dprimeVals(11:12), 'LineWidth', 0.5, 'Color', [0 0 0]);
icDataTestsOnly{2,39} = [dp_r; dp_ocFull];
title('Choice');ylim([-1 4]);
xticklabels({'','light off','light on'});

dprimeFig.Position(3:4)=[725 225];
saveas(gcf,['Fibers_T_IC_dp_bar_Opto']);
saveas(gcf,['Fibers_T_IC_dp_bar_Opto.png']);
saveas(gcf,['Fibers_T_IC_dp_bar_Opto.pdf']);
