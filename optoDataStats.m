...
cohortRange=[1:3,6]; % now load the data for each cohort and make one big file called allCohorts
...
condition = reshape([expList'; zeros(1,numel(expList))],1,[]);
condition = [0 0 condition];
testIdx=find(condition==1); allDataTestsOnly=allCohorts(1,:); lickMatTestsOnly=allLickMat(1,:);
lickIdx=[0 expList'];
lickIdx=find(lickIdx==1);
...
allDataTestsOnly(8,:)=[];lickMatTestsOnly(8,:)=[];allLicksTest(7,:)=[];
...
% icDataCtlOnly(3,:)=allCohorts(77,:);
% icDataCtlOnly(4,:)=allCohorts(79,:);
% 
% mgmOnly(1,:)=allCohorts(1,:);
% mgmOnly(2,:)=allCohorts(82,:);
% mgmOnly(3,:)=allCohorts(84,:);
% mgmOnly(4,:)=allCohorts(86,:);

[allDataTestsOnly,icDataTestsOnly]=makeHeaders(allDataTestsOnly,icDataTestsOnly);
...
if icCtlData==0
    ...
    for jj=2:size(icDataCtlOnly,1) % control animals
    eeFig=figure(jj+10);hold on;
    rhit=icDataCtlOnly{jj,18}; % full IC
    rfa=icDataCtlOnly{jj,19};
    ohit = icDataCtlOnly{jj,20};
    ofa = icDataCtlOnly{jj,21};
    rhit(isnan(ohit))=nan;
    rfa(isnan(ofa))=nan;
    rpc = (rhit+(1-rfa))/2*100; 
    opc = (ohit+(1-ofa))/2*100; 
    ...
    sigstar({[1,2]}, p)
    ylabel('percent correct');
    title([icDataCtlOnly{jj,1} ' IC Full Trial Inactivation']);
    xticklabels({'off', 'on'});
    icDataCtlOnly{jj,33}=rpc;
    icDataCtlOnly{jj,34}=opc;

    rhit=icDataCtlOnly{jj,18}; % tone IC
    rfa=icDataCtlOnly{jj,19};
    ohit = icDataCtlOnly{jj,22};
    ofa = icDataCtlOnly{jj,23};
    rhit(isnan(ohit))=nan;
    rfa(isnan(ofa))=nan;
    rpc = (rhit+(1-rfa))/2*100; 
    opc = (ohit+(1-ofa))/2*100; 
    ...
    [h,p,ci,stats] = ttest2(rpc,opc);
    sigstar({[1,2]}, p)
    title([icDataCtlOnly{jj,1} ' IC Tone Inactivation']);
    xticklabels({'off', 'on'});
    icDataCtlOnly{jj,35}=rpc;
    icDataCtlOnly{jj,36}=opc;

    rhit=icDataCtlOnly{jj,18}; % choice IC
    rfa=icDataCtlOnly{jj,19};
    ohit = icDataCtlOnly{jj,24};
    ofa = icDataCtlOnly{jj,25};
    rhit(isnan(ohit))=nan;
    rfa(isnan(ofa))=nan;
    rpc = (rhit+(1-rfa))/2*100; 
    opc = (ohit+(1-ofa))/2*100; 
    icDataCtlOnly{jj,37}=rpc;
    icDataCtlOnly{jj,38}=opc;
    ...
    [h,p,ci,stats] = ttest2(rpc,opc);
    sigstar({[1,2]}, p)
    title([icDataCtlOnly{jj,1} ' IC Choice Inactivation']);
    xticklabels({'off', 'on'});
    
    eeFig.Position(3:4)=[725 475];
    saveas(gcf,[char(icDataCtlOnly{jj,1}) '_C_IC_PercentCorrect_Opto']);
    saveas(gcf,[char(icDataCtlOnly{jj,1}) '_C_IC_PercentCorrect_Opto.png']);
    end
end
...
plotMGM=0;
if plotMGM==1
    for jj=2:size(mgmOnly,1)
        eeFig=figure(jj);hold on;
        rhit=mgmOnly{jj,10}; % full trial MGB
        rfa=mgmOnly{jj,11};
        ohit = mgmOnly{jj,12};
        ofa = mgmOnly{jj,13};
        rhit(isnan(ohit))=nan;
        ...
        sigstar({[1,2]}, p)
        ylabel('percent correct');
        title([mgmOnly{jj,1} ' MGM Full Trial Inactivation']);
        xticklabels({'off', 'on'});
        mgmOnly{jj,27}=rpc;
        mgmOnly{jj,28}=opc;

        rhit=mgmOnly{jj,10}; % tone MGB
        rfa=mgmOnly{jj,11};
        ohit = mgmOnly{jj,14};
        ofa = mgmOnly{jj,15};
        rhit(isnan(ohit))=nan;
        ...
        [h,p,ci,stats] = ttest2(rpc,opc);
        sigstar({[1,2]}, p)
        title([mgmOnly{jj,1} ' MGM Tone Inactivation']);
        xticklabels({'off', 'on'});
        mgmOnly{jj,29}=rpc;
        mgmOnly{jj,30}=opc;

        rhit=mgmOnly{jj,10}; % choice MGB
        rfa=mgmOnly{jj,11};
        ohit = mgmOnly{jj,16};
        ofa = mgmOnly{jj,17};
        rhit(isnan(ohit))=nan;
        ...
        [h,p,ci,stats] = ttest2(rpc,opc);
        sigstar({[1,2]}, p)
        title([mgmOnly{jj,1} ' MGM Choice Inactivation']);
        xticklabels({'off', 'on'});

        eeFig.Position(3:4)=[725 475];
        saveas(gcf,[char(mgmOnly{jj,1}) '_T_MGM_PercentCorrect_Opto']);
        saveas(gcf,[char(mgmOnly{jj,1}) '_T_MGM_PercentCorrect_Opto.png']);    
    end
else
end
...
if plotMGM==1
    for jj=2:size(mgmOnly,1)
        ...
        xticklabels({'hit','fa','hit','fa'});
        eeFig.Position(3:4)=[725 475];
        saveas(gcf,[char(mgmOnly{jj,1}) '_T_MGM_HitFARate_Opto']);
        saveas(gcf,[char(mgmOnly{jj,1}) '_T_MGM_HitFARate_Opto.png']);
    end
else
end
...
%% plot ROC curve with d' and criterion

rocPlots(mgbTempTestsOnly);

%% get learning curves...
% load rates variable for each animal, and specify the 
clear expertHit expertFA dprime
for pp=2:size(rates)
    rateTemp=rates{pp,2};
    expdays=days{pp,4}(1)-1;
    expertHit(pp-1)=rateTemp(expdays,1); 
    expertFA(pp-1)=rateTemp(expdays,2);
    dprime(pp-1)=( norminv(expertHit(pp-1)-0.001) - norminv(expertFA(pp-1)) )';  
end
perfFig=figure;
perfFig.Position(3:4)=[350,170];
        hitcolor=[0.47,0.67,0.3];
        facolor=[0.93,0.69,0.3];
    subplot(1,2,1); 
    dps=scatter(ones(1),mean(dprime),'filled');hold on;
    dps.MarkerFaceColor=[0.4 0.4 0.4];
    errorbar(mean(dprime),std(dprime),'Color',[0.4 0.4 0.4]);
    ylim([0 4]);xlim([0.5 1.5]);ylabel('d prime'); box off;
    subplot(1,2,2);
    scatter(ones(1),mean(expertHit),...
        'MarkerFaceColor',hitcolor);hold on;
    errorbar(mean(expertHit),std(expertHit),'Color',hitcolor);
    scatter(ones(1),mean(expertFA),...
        'MarkerFaceColor',facolor);hold on;
    ylabel('Action Rate');
    errorbar(mean(expertFA),std(expertFA),'Color',facolor);
