function data=bpod_data_preprocessing(rawdata_folder,GNG_protocol,Animals)
...
%     foldertouse=[folder Animals(f1) filesep GNG_protocol filesep]; %[folder cell2mat(Animals(fl))  filesep 'LickingGNG_Quarter' filesep]; % 'LickingGNG_Normal [folder cell2mat(Animals(fl)) '/LickingGNG_Quarter/'];
    ...
    for fii= 1:length(matfiles2) %for all files from selected animal
        ...
        data(fl).dprime{fii}=[dp_r;dp_p]; % dprime row 1 is reinforced, 2 is probe
